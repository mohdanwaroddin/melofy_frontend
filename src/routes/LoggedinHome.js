...
const Card = ({ info, playSound, name, firstName, thumbnail }) => {
    ...
    return (
        <div className="bg-black bg-opacity-40 w-full p-4 rounded-lg
         hover:bg-opacity-90 transition duration-500 cursor-pointer "
            onClick={() => {
                setCurrentSong(info);
            }}
        >
            <div className="pb-4 ">
                <img className="w-full h-40 rounded-md" src={thumbnail} alt="label" />
            </div>
            <div className="text-white font-semibold hover:underline">{name}</div>
            <div className="text-gray-500 text-sm hover:underline">{firstName}</div>
